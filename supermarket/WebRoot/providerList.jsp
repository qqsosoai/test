...
function sh(id){
	$(".zhezhao").show();
	$("#removeProv").show();
	$("#yes").attr("href","Provider?method=delete&id="+id);
}
$(function(){
	var flag=getFirstParam();
	if(flag=='false'){alert("删除失败，请先删除其账单")}
	if(flag=='insFalse'){alert("添加失败，请重新添加")}
	if(flag=='upFalse'){alert("修改失败，请重新修改该")}
 	var pageIndex="";//当前页码
	var pageSize="";//页大小
	var pageCount="";//总页数
	var sqlCount="";//总记录数
	...
	function init(pageIndex){
		$.post("Provider",
				{"method":"select","pageIndex":pageIndex,"search":$("input:first").val()},
				f,"json");
	}
	function f(data){
		$("tbody").html("");
		$(data).each(function(){
			if(this.id){//代表为供应商数据
				$('tbody').append("<tr><td>"+this.proCode+"</td>"
		         +"       <td>"+this.proName+"</td>"
		         ...
		         +"       <td>                                                                                        "
		         +"           <a href='providerView.jsp?id="+this.id+"'><img src='img/read.png' alt='查看' title='查看'/></a>           "
		         +"           <a href='providerUpdate.jsp?id="+this.id+"'><img src='img/xiugai.png' alt='修改' title='修改'/></a>       "
		         +"           <a href='javascript:sh("+this.id+")' class='removeProvider'><img src='img/schu.png' alt='删除' title='删除'/></a>"
		         +"       </td>                                                                                       "
		         +"   </tr>");
	         }else if(this.flag){
